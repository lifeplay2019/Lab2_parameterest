...
def run_rls_identification(
    ...
    plot_results_enhanced(rls, t[1:], SOC[1:], Uocv[1:], Ts[1:])
...
def plot_results_enhanced(rls, t, SOC, Uocv, Ts_measured):
    ...
    ax1.set_ylabel('Cc (J/K)')
    ax1.set_title('Core Heat Capacity Identification')
    ...
    ax2.plot(t_plot, theta_smooth_history[:, 1], 'r-', linewidth=2.5, label='Rc (smoothed)')
    ax2.plot(t_plot, theta_smooth_history[:, 2], 'g-', linewidth=2.5, label='Rs (smoothed)')
    ax2.set_xlabel(t_label)
    ax2.set_ylabel('Thermal Resistance (K/W)')
    ax2.set_title('Thermal Resistance Identification')
    ...
    ax3.fill_between(t_plot, Ts_measured, Ts_pred, alpha=0.2, color='orange', label='Prediction Error')
    ax3.set_xlabel(t_label)
    ax3.set_ylabel('Surface Temperature (°C)')
    ax3.set_title('Temperature Prediction Comparison')
    ...
    ax7.semilogy(t_plot, rls.condition_number_history, 'orange', linewidth=2)
    ax7.set_xlabel(t_label)
    ax7.set_ylabel('Condition Number')
    ax7.set_title('Covariance Matrix Condition Number')
...
def plot_performance_metrics(rls, t_plot):
    ...
    if len(residuals) > window:
        ...
        axes[0, 0].set_ylabel('Error Metric (°C)')
        axes[0, 0].set_title('Sliding Window Error Metrics')
        axes[0, 0].legend()
    ...
    if len(theta_smooth) > window:
        ...
        axes[0, 1].set_ylabel('CV of Cc')
        axes[0, 1].set_title('Parameter Convergence (Cc)')
        axes[0, 1].legend()
    ...
    if len(residuals) > 50:
        ...
        axes[1, 0].axhline(-0.2, color='r', linestyle='--', alpha=0.7)
