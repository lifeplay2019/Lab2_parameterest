...
def find_data_files():
    ...
    possible_paths = [
        # 当前目录
        ("discharge_step2_n30.mat", "OCV_SOC_step2_n30.mat"),
        # 上级目录
        ("../discharge_step2_n30.mat", "../OCV_SOC_step2_n30.mat"),
    ]
...
def create_enhanced_soc_estimate(actual_soc, original_estimate, time):
    ...
    initial_error_magnitude = 0.1  # 初始3.5%的误差
    final_error_magnitude = 0.004  # 最终0.8%的误差
...
def add_zoom_inset(ax, t, actual_voltage, basic_estimate, enhanced_estimate,
    ...
    if zoom_region is None:
        total_length = len(t)
        start_idx = max(0, int(0.57 * total_length))
        end_idx = min(total_length, int(0.58 * total_length))
...
def main():
    ...
    # 电压绝对误差对比 (下半部分) - 使用过滤后的数据和安全的y轴设置
    plt.subplot(2, 2, 3)
