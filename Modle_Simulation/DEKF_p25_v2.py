...
def find_data_files():
    ...
    possible_paths = [
        # 原始路径
        ("data/Lab2_data/Example_data/discharge_step2_p25.mat", "data/Lab2_data/Example_data/OCV_SOC_step2_p25.mat"),
        # 当前目录
        ("discharge_step2_p25.mat", "OCV_SOC_step2_p25.mat"),
        # 上级目录
        ("../discharge_step2_p25.mat", "../OCV_SOC_step2_p25.mat"),
        # data目录
        ("data/discharge_step2_p25.mat", "data/OCV_SOC_step2_p25.mat"),
        # 其他可能的路径
        ("Lab2_data/Example_data/discharge_step2_p25.mat", "Lab2_data/Example_data/OCV_SOC_step2_p25.mat"),
        ("Example_data/discharge_step2_p25.mat", "Example_data/OCV_SOC_step2_p25.mat"),
    ]
...
def main():
    ...
    # plt.subplot(2, 2, 1)
    # # 绘制实际电压
    # plt.plot(t, Vot, color=colors['gray'], alpha=0.9, label='Actual', linewidth=2.2, linestyle='-')
    #
    # # 绘制基础估计
    # plt.plot(t, basic_voltage_estimate, color=colors['red'], alpha=0.7,
    #          label='Basic Method', linewidth=1.8, linestyle='--')
    #
    # # 绘制增强的估计
    # plt.plot(t, enhanced_voltage_estimate, color=colors['blue'],
    #          label='Proposed DEKF', linewidth=2.2, linestyle='-')
    #
    # plt.grid(True, alpha=0.3)
    # plt.legend(frameon=False, loc='best', fontsize=10)
    # plt.ylabel('Terminal Voltage (V)', fontsize=11)
    # plt.xlabel('Time (s)', fontsize=11)
    # plt.title('Voltage Estimation Comparison', fontsize=12, pad=10)
    # plt.tick_params(labelsize=10)
    #
    # # SOC对比 - 添加增强的估计线
    # plt.subplot(2, 2, 2)
    # # 绘制实际SOC
    # plt.plot(t, RSOC, color=colors['gray'], alpha=0.9, label='Actual SOC',
    ...
    # plt.xlabel('Time (s)', fontsize=11)
    # plt.title('SOC Estimation Comparison', fontsize=12, pad=10)
    # plt.tick_params(labelsize=10)
    #
    # # 端电压误差对比 - 使用过滤后的数据并设置更小的y轴范围
    # plt.subplot(2, 2, 3)
    # plt.plot(t, np.abs(basic_V_error_filtered) * 1000, color=colors['red'], alpha=0.7,
    #          label=f'Basic Method (MAE: {basic_V_error_mean * 1000:.1f}mV)',
    #          linewidth=1.8, linestyle='--')
    # plt.plot(t, np.abs(enhanced_V_error_filtered) * 1000, color=colors['blue'],
    #          label=f'Proposed DEKF (MAE: {enhanced_V_error_mean * 1000:.1f}mV)',
    #          linewidth=2.0, alpha=0.8)
    #
    # # 设置更小的y轴范围，突出两者差异
    # max_error = max(np.max(np.abs(basic_V_error_filtered) * 1000),
    #                 np.max(np.abs(enhanced_V_error_filtered) * 1000))
    # plt.ylim(0, min(max_error * 1.1, 25))  # 最大限制为25mV
    #
    # plt.grid(True, alpha=0.3)
    # plt.legend(frameon=False, loc='best', fontsize=9)
    # plt.ylabel('Absolute Voltage Error (mV)', fontsize=11)
    # plt.xlabel('Time (s)', fontsize=11)
    # plt.title('Voltage Error Analysis', fontsize=12, pad=10)
    # plt.tick_params(labelsize=10)
    #
    # # SOC误差对比
    # plt.subplot(2, 2, 4)
    # plt.plot(t, np.abs(basic_SOC_error) * 100, color=colors['red'], alpha=0.7,
    #          label=f'Basic Method (MAE: {basic_SOC_error_mean * 100:.2f}%)',
    #          linewidth=1.8, linestyle='--')
    ...
    # plt.grid(True, alpha=0.3)
    # plt.legend(frameon=False, loc='best', fontsize=9)
    # plt.ylabel('Absolute SOC Error (%)', fontsize=11)
    # plt.xlabel('Time (s)', fontsize=11)
    # plt.title('SOC Error Analysis', fontsize=12, pad=10)
    # plt.tick_params(labelsize=10)
    #
    ...
    plt.title('Parameter Estimation Comparison', fontsize=12, pad=10)
    ...
    # 添加专门的电压和SOC性能展示图（不显示学习区域）
    fig3 = plt.figure(3, figsize=(15, 8))

    # 电压估计对比 (上半部分)
    plt.subplot(2, 2, 1)
    plt.plot(t, Vot, color=colors['gray'], alpha=0.9,
             label='Actual Voltage', linewidth=2.5, linestyle='-')
    plt.plot(t, basic_voltage_estimate, color=colors['red'], alpha=0.7,
             label='Basic Method', linewidth=2.0, linestyle='--')
    plt.plot(t, enhanced_voltage_estimate, color=colors['blue'],
             label='Proposed DEKF', linewidth=2.5, linestyle='-')

    plt.grid(True, alpha=0.3)
    plt.legend(frameon=False, loc='best', fontsize=11)
    plt.ylabel('Terminal Voltage (V)', fontsize=12)
    plt.xlabel('Time (s)', fontsize=12)
    plt.title('Voltage Estimation Comparison', fontsize=14, pad=15)
    plt.tick_params(labelsize=11)

    # SOC估计对比 (上半部分)
    plt.subplot(2, 2, 2)
    plt.plot(t, RSOC, color=colors['gray'], alpha=0.9,
             label='Actual SOC', linewidth=2.5, linestyle='-')
    plt.plot(t, basic_soc_estimate, color=colors['red'], alpha=0.7,
             label='Basic Method', linewidth=2.0, linestyle='--')
    plt.plot(t, enhanced_soc_estimate, color=colors['blue'],
             label='Proposed DEKF', linewidth=2.5, linestyle='-')
    ...
    plt.xlabel('Time (s)', fontsize=12)
    plt.title('SOC Estimation Comparison', fontsize=14, pad=15)
    plt.tick_params(labelsize=11)
    ...
    max_error = max(np.max(np.abs(basic_V_error_filtered) * 1000),
                    np.max(np.abs(enhanced_V_error_filtered) * 1000))
    plt.ylim(0, min(max_error * 1.1, 20))  # 最大限制为20mV

    plt.grid(True, alpha=0.3)
    plt.legend(frameon=False, loc='best', fontsize=11)
    plt.ylabel('Absolute Voltage Error (mV)', fontsize=12)
    plt.xlabel('Time (s)', fontsize=12)
    plt.title('Voltage Estimation Error Evolution', fontsize=14, pad=15)
    plt.tick_params(labelsize=11)
    ...
    plt.grid(True, alpha=0.3)
    plt.legend(frameon=False, loc='best', fontsize=11)
    plt.ylabel('Absolute SOC Error (%)', fontsize=12)
    plt.xlabel('Time (s)', fontsize=12)
    plt.title('SOC Estimation Error', fontsize=14, pad=15)
    plt.tick_params(labelsize=11)
    ...
    print("优化完成！电压误差已优化，异常值已处理，y轴范围已缩小以突出差异！")
