...
def find_data_files():
    ...
    possible_paths = [
        # 当前目录
        ("discharge_step2_n15.mat", "OCV_SOC_step2_n15.mat"),
        # 上级目录
        ("../discharge_step2_n15.mat", "../OCV_SOC_step2_n15.mat"),
    ]
...
def add_zoom_inset(ax, t, actual_voltage, basic_estimate, enhanced_estimate,
    ...
    if zoom_region is None:
        total_length = len(t)
        start_idx = max(0, int(0.65 * total_length))
        end_idx = min(total_length, int(0.66 * total_length))
...
def main():
    ...
    if max_error > 0 and np.isfinite(max_error):
        plt.ylim(0, min(max_error * 1.1, 40))
    else:
        plt.ylim(0, 5)  # 默认范围
