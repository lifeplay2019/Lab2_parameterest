...
def main():
    ...
    # 端电压对比 - 更扁的子图
    plt.subplot(2, 2, 1)
    plt.plot(t, Vot, color=colors['gray'], alpha=0.8, label='Actual', linewidth=1.8)
    plt.plot(t, Vekf, color=colors['blue'], label='Estimated', linewidth=1.5)
    ...
    # SOC对比
    plt.subplot(2, 2, 2)
    plt.plot(t, RSOC, color=colors['gray'], alpha=0.8, label='Actual', linewidth=1.8)
    plt.plot(t, Xekf_all[2, :], color=colors['red'], label='Estimated', linewidth=1.5)
    plt.grid(True, alpha=0.3)
    plt.legend(frameon=False, loc='best')
    ...
    # 端电压误差 - 扩大y轴范围以显示误差的改善
    plt.subplot(2, 2, 3)
    plt.plot(t, V_error, color=colors['green'], linewidth=1.5, alpha=0.8)
    plt.grid(True, alpha=0.3)
    plt.ylabel('Voltage Error (V)', fontsize=11)
    plt.xlabel('Time (s)', fontsize=11)
    plt.title('', fontsize=12, pad=10)
    plt.tick_params(labelsize=10)
    # 扩大y轴范围以便更好地观察误差变化
    ...
    plt.ylim(current_ylim[0] - y_range * 0.3, current_ylim[1] + y_range * 0.3)

    # SOC误差 - 扩大y轴范围
    plt.subplot(2, 2, 4)
    plt.plot(t, SOC_error, color=colors['orange'], linewidth=1.5, alpha=0.8)
    plt.grid(True, alpha=0.3)
    plt.ylabel('SOC Error (%)', fontsize=11)
    plt.xlabel('Time (s)', fontsize=11)
    plt.title('', fontsize=12, pad=10)
    plt.tick_params(labelsize=10)
    # 扩大y轴范围以便更好地观察误差变化
