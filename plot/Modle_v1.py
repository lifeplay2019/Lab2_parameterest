...
def main():
    ...
    if not os.path.exists(discharge_file):
        print(f"错误：找不到放电数据文件: {discharge_file}")
        # 让我们看看这个目录下有什么文件
        dir_path = os.path.dirname(discharge_file)
    ...
    if not os.path.exists(ocv_file):
        print(f"错误：找不到OCV数据文件: {ocv_file}")
        # 让我们看看这个目录下有什么文件
        dir_path = os.path.dirname(ocv_file)
        if os.path.exists(dir_path):
            print(f"目录 {dir_path} 中的文件:")
            for file in os.listdir(dir_path):
                print(f"  {file}")
    ...
    try:
        ...
        # 查找放电数据
        possible_discharge_keys = ['discharge', 'discharge_step2', 'data', 'discharge_data']
        ...
        # 查找OCV数据
        possible_ocv_keys = ['OCV_SOC', 'OCV_SOC_step2', 'ocv_soc', 'data', 'ocv_data']
        ...
        if discharge_data is None:
            print("错误：无法从discharge_step2.mat中找到数据")
            return

        if ocv_soc_data is None:
            print("错误：无法从OCV_SOC_step2.mat中找到数据")
            return
